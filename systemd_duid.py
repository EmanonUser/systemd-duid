...
def generate_duid_llt(mac_address: str) -> str:
    ...
    duid_systemd_default_time: str = '00000000'              # Assume default time 2000-01-01 00:00:00 UTC
...
def generate_duid_en(machine_id: str, app_id: str) -> str:
    ...
    return duid_type_en + systemd_entreprise_id + inverted_hash.hex()
...
def generate_duid_uuid(machine_id: str, application_id_uuid: str) -> str:
    ...
    machine_id: bytes = bytes.fromhex(machine_id)
    application_id_uuid: bytes = bytes.fromhex(application_id_uuid)
